...
def ocr_pdf_text(pdf_path: Path, lang: str = "eng") -> str:
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        try:
            subprocess.run(
                ["pdftoppm", str(pdf_path), str(tmp_prefix), "-png"],
                check=True,
            ...
            print(
                f"OCR dependency missing: 'pdftoppm' not found. Skipping OCR for {pdf_path}",
                file=sys.stderr,
            )
            return ""
        ...
        for img in sorted(Path(tmpdir).glob("page-*.png")):
            ...
            try:
                ...
                print(
                    f"OCR dependency missing: 'tesseract' not found. Skipping OCR for {pdf_path}",
                    file=sys.stderr,
                )
                return text
            except subprocess.CalledProcessError as e:
